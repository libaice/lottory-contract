    ...
    : describe("Raffle Unit Tests", function () {
        ...
        describe("checkUpkeep", function () {
            it("returns false if people haven't sent any ETH", async () => {
                ...
                // call static is send Preexec tx
                const {upkeepNeeded} = await raffle.callStatic.checkUpkeep("0x") // upkeepNeeded = (timePassed && isOpen && hasBalance && hasPlayers)
                console.log(" upkeepNeeded result is  ", upkeepNeeded)
                assert(!upkeepNeeded)
            })

            it("returns false if raffle isn't open", async () => {
                console.log("============================================")
                const enterRaffle = await raffle.enterRaffle({value: raffleEntranceFee})
                console.log(enterRaffle.hash)
                await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                await network.provider.request({method: "evm_mine", params: []})

                const upKeepId = await raffle.performUpkeep([]) // changes the state to calculating
                console.log("performUpKeep is  ", upKeepId.hash)
